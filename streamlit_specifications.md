...
# Streamlit Application Specification: AI Talent Demand Tracker

## 1. Application Overview

The "AI Talent Demand Tracker" Streamlit application serves as a comprehensive tool for HR and strategy teams to gain crucial insights into the evolving demand for AI talent in the broader market. Built by a Software Developer, this application simulates the entire workflow of collecting, processing, analyzing, and visualizing job market data related to AI skills and roles. Its purpose is to empower the organization with actionable intelligence, enabling proactive adaptation of hiring strategies, identification of in-demand skills, and a clear understanding of the competitive landscape for AI professionals.

The application guides the user through a simulated multi-stage workflow, demonstrating the practical application of data processing and analysis concepts:

1.  **Introduction & Data Generation**: The user starts by understanding the lab's purpose and then simulates the generation of raw job posting data, mirroring real-world data collection challenges.
2.  **Defining AI Taxonomy & Scoring Logic**: This section focuses on the core intelligence of the system. The user explores and tests the predefined AI skill taxonomy, seniority classification logic, and the AI Relevance Score calculation using sample data. This highlights how raw text is transformed into quantifiable features.
3.  **Data Processing & Deduplication**: The application demonstrates how the defined logic is applied to the entire dataset. A crucial step of deduplication is performed to ensure data quality, simulating real-world data cleaning requirements.
4.  **Temporal Aggregation & Visualizations**: The final stage aggregates the clean, processed data on a weekly basis, preparing it for trend analysis. The application then presents key insights through interactive charts, showing weekly job volume trends, top requested AI skills, and demand distribution across seniority levels. This allows HR to visualize and interpret complex market dynamics at a glance.

The app is designed to provide a hands-on experience for a Software Developer, illustrating how to construct such a system from fundamental components to a complete, insightful dashboard.
...
from io import BytesIO # To handle image downloads
...
st.set_page_config(layout="wide", page_title="AI Talent Demand Tracker")
...
page_selection = st.sidebar.radio(
    ...
    index=["Introduction & Data Generation", "Defining AI Taxonomy & Scoring Logic", "Data Processing & Deduplication", "Temporal Aggregation & Visualizations"].index(st.session_state.current_page)
)
...
st.title("PE-Org AIR System: AI Talent Demand Tracker")
...
# --- Page: Defining AI Taxonomy & Scoring Logic ---
    ...
    st.json({k.value: list(v)[:3] + ['...'] for k, v in AI_SKILLS.items()}) # Display truncated AI_SKILLS
    ...
    st.code("def extract_ai_skills(text: str) -> Set[str]:\n    # ... implementation details ...") # Simplified as implementation is in source.py
    ...
    st.markdown(r"The formula for the AI Relevance Score (ranging from 0 to 1) is:")
    st.markdown(r"$$ \text{{min}}(\frac{{\text{{len(skills)}}}}{{5}}, 1.0) * 0.6 + (0.4 \text{{ if AI keywords present in title else }} 0.0) $$")
    st.markdown(r"where $ \text{{len(skills)}} $ is the number of unique AI skills extracted from the job description.")
    st.markdown(r"The division by $5$ normalizes the skill count to a maximum of $1.0$ if $5$ or more unique skills are found.")
    st.markdown(r"The $0.6$ and $0.4$ weights represent the contribution of skills and title keywords, respectively.")
        ...
        if st.session_state.weekly_job_volume_df is not None:
            ...
            if st.button("Generate Visualizations", key="generate_viz_btn"):
                ...
                with st.spinner("Generating weekly job volume plot..."):
                    ...
                    st.pyplot(plt.gcf()) # Display the generated matplotlib figure
